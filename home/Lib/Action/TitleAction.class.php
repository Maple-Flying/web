    ...
    public function index(){
		...
		if(session('?username')){
		  ...
		  $tid=$_GET["id"];
            ...
            $page->setConfig('last', 'Last &raquo;');//最后一页				$page->setConfig('theme',' %first% %upPage%  %linkPage%  %downPage% %end%');
            //设置分页回调方法
			$show=$page->show();
			$news_list=$allnews->where("tid=$tid")->order('id desc')->limit($page->firstRow.','.$page->listRows)->select();

            $mid = $title->where("id=$tid")->find();
            $ttt = $mid['title'];

            $mem = $group->where("title='".$ttt."'")->select();
